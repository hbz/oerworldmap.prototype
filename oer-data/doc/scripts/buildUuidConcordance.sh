...
for i in $(ls ../../output/wsis/geo/w/*); do
...
rm $CONCORDANCE_FILE_TMP
for i in $(ls ../../output/wsis/wsis-initiative-data.json/w/*); do
...
# internalWsisId -> uuid
grep '1.1/identifier' ../../output/oerWorldmapTestResult.nt | sed -e 's#.*oer/\(.*\)\#!> <http://purl.org/dc/elements/1.1/identifier> "\(.*\)".*#\2\t\1#'  >>  $CONCORDANCE_FILE_TMP
# internalOcwcId -> uuid
...
for i in $(ls ../../output/geo/ocwc/geoList/o/ocwc*); do
  uuid=$(grep "schema.org/address>" $i | cut -d ' ' -f3 | sed -e 's#.*:.*:\(.*\)>#\t\1#')
  echo "$(basename $i | cut -d '.' -f 1)Address$uuid" >> $CONCORDANCE_FILE_TMP
done

# serviceUrls -> uuid
for i in $(ls ../../output/ocwc/organizationId/o/ocwc*); do
